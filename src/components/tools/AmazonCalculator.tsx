...
export default function AmazonCalculator() {
  ...
  useEffect(() => {
    // Basic Amazon India Fee Logic (Simplified for Demo)
    const referralRate = category === 'Electronics' ? 0.08 : 0.12;
    const referralFee = sellingPrice * referralRate;
    
    let closingFee = 0;
    if (sellingPrice <= 250) closingFee = 2;
    else if (sellingPrice <= 500) closingFee = 5;
    else if (sellingPrice <= 1000) closingFee = 30;
    else closingFee = 60;

    const shippingFee = weight * 45; // Base rate per 500g
  ...
  }, [sellingPrice, costPrice, weight, category]);
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className="w-full h-2 bg-gray-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-primary-blue"
            />
            <div className="flex justify-between text-[10px] font-bold text-gray-400 mt-2">
              <span>0.1kg</span>
              <span className="text-primary-blue">{weight}kg</span>
              <span>10kg</span>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-900/30 flex items-start gap-3 text-blue-700 dark:text-blue-400 text-xs leading-relaxed">
            ...
            <p>Fees are estimated based on standard Amazon India Easy Ship rates. Actual fees may vary based on your seller tier and specific sub-category.</p>
          </div>
        </div>

        <div className="space-y-6">
          <div className="p-6 rounded-2xl bg-gray-50 dark:bg-slate-800/50 border border-gray-100 dark:border-slate-700">
            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-6">Fee Breakdown</h3>
            
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-slate-400">Referral Fee</span>
                <span className="text-sm font-bold dark:text-white">₹{fees.referral.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-slate-400">Closing Fee</span>
                <span className="text-sm font-bold dark:text-white">₹{fees.closing.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center">
