...
export default function FlipkartCalculator() {
  ...
  const [sellerTier, setSellerTier] = useState('Bronze');
  ...
  useEffect(() => {
    // Flipkart India Fee Logic (Simplified)
    const commissionRate = category === 'Electronics' ? 0.05 : 0.10;
    const commission = sellingPrice * commissionRate;
    
    let fixedFee = 0;
    if (sellingPrice <= 300) fixedFee = 13;
    else if (sellingPrice <= 500) fixedFee = 15;
    else if (sellingPrice <= 1000) fixedFee = 25;
    else fixedFee = 45;

    const collectionFee = sellingPrice * 0.02; // 2% for prepaid/COD
    const shippingFee = weight * 40; // Base rate
    
    const totalFeesBeforeGst = commission + fixedFee + collectionFee + shippingFee;
  ...
  }, [sellingPrice, costPrice, weight, category, sellerTier]);
      ...
      <div className="grid md:grid-cols-2 gap-12">
        <div className="space-y-6">
          ...
          <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-900/30 flex items-start gap-3 text-blue-700 dark:text-blue-400 text-xs leading-relaxed">
            ...
            <p>Calculations include Commission, Fixed Fee, Collection Fee, and Shipping. GST of 18% is applied on all marketplace fees.</p>
          </div>
        </div>

        <div className="space-y-6">
          <div className="p-6 rounded-2xl bg-gray-50 dark:bg-slate-800/50 border border-gray-100 dark:border-slate-700">
            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-6">Flipkart Fee Breakdown</h3>
            
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-slate-400">Marketplace Commission</span>
                <span className="text-sm font-bold dark:text-white">₹{fees.commission.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-slate-400">Fixed Fee</span>
                <span className="text-sm font-bold dark:text-white">₹{fees.fixed.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-slate-400">Collection Fee</span>
                <span className="text-sm font-bold dark:text-white">₹{fees.collection.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-slate-400">GST on Fees (18%)</span>
